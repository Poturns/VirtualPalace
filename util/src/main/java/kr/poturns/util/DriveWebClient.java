 ...
 */
public class DriveWebClient {


    public static final String FOLDER_ID_ROOT = "root";
    private static final String FOLDER_MINE = "application/vnd.google-apps.folder";
    ...
    public DriveWebClient(Context context) {
    ...
    public List<File> requestChildFileList(String folderId) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            batchRequest.queue(setPartialRequest(mFilesMethodSet.get(childReference.getId())).buildHttpRequest(), File.class, Void.class, new BatchCallback<File, Void>() {
                @Override
                public void onSuccess(File file, HttpHeaders responseHeaders) throws IOException {
                    if (file != null && file.getTitle() != null && !file.getExplicitlyTrashed() && !file.getTitle().startsWith("~$") && !file.getTitle().endsWith("[충돌]")) {
    ...
    public List<File> requestParentList(List<ParentReference> parentReferences) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            batchRequest.queue(setPartialRequest(mFilesMethodSet.get(parentReference.getId())).buildHttpRequest(), File.class, Void.class, new BatchCallback<File, Void>() {
                @Override
                public void onSuccess(File file, HttpHeaders responseHeaders) throws IOException {
                    synchronized (fileList) {
                        fileList.add(file);
...
/*
    public File getFile(String fileId) throws IOException {
        return setPartialRequest(mFilesMethodSet.get(fileId)).execute();
    }
...
*/
     ...
     */
    private static Drive.Files.Get setPartialRequest(Drive.Files.Get request) {
    ...
    public static boolean isFolder(File file) {
        return file.getMimeType().equals(FOLDER_MINE);
    }
